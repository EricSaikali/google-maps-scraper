...
def parse_json_data(json_str):
    ...
    try:
        ...
        if isinstance(initial_data, list) and len(initial_data) > 3 and isinstance(initial_data[3], list) and len(
                initial_data[3]) > 6:
                ...
                try:
                    ...
                    if isinstance(actual_data, list) and len(actual_data) > 6:
                        ...
                        if isinstance(potential_data_blob, list):
                            ...
                            print(
                                f"Data at actual_data[6] is not a list, but {type(potential_data_blob)}. Saving inner data for inspection.")
                        ...
                        print(
                            f"Parsed inner JSON is not a list or too short (len <= 6), type: {type(actual_data)}. Saving inner data for inspection.")
                        # Save actual_data for debugging
                        try:
                            with open("debug_inner_data.json", "w", encoding="utf-8") as f_inner:
                                json.dump(actual_data, f_inner, indent=2)
...
def flatten(data, storage: List[str]):
    if data is None:
        pass
...
def remove_nulls(obj, *, drop_empty: bool = True):
    """
    Recursively traverse `obj` (list / dict / scalar) and remove:
      • list items that are None
      • dict keys whose (cleaned) value is None
    If `drop_empty` is True, empty lists/dicts are replaced with None
    (so they will be pruned by their parent container).

    Returns the cleaned structure (or None if everything was removed).
    """
    # Handle lists -----------------------------------------------------------
    if isinstance(obj, list):
        cleaned = [remove_nulls(item, drop_empty=drop_empty)
                   for item in obj]
        cleaned = [item for item in cleaned if item is not None]  # drop Nones
        if cleaned or not drop_empty:
            return cleaned
        return None  # prune empty

    # Handle dictionaries ----------------------------------------------------
    if isinstance(obj, dict):
        cleaned = {k: remove_nulls(v, drop_empty=drop_empty)
...
def is_clean_text(
        ...
        vocabulary: Set[str],
        threshold: float = 0.6,
        min_num_word: float = 10
) -> bool:
    """
    Returns True if ALL of the following hold:
      1. text contains NO spaces
      2. text does NOT contain "http"
      3. at least `threshold` fraction of its characters can be
         covered by vocabulary words when optimally segmented.
    """
    # 1) no spaces
    if ' ' not in text:
        return False
    ...
    words = text.split()
    count = len([1 for word in words if word.lower().strip() in vocabulary])
    length = len(words)
    if count < min_num_word:
        return False
...
def get_reviews(data_blob, vocab):
    """
    ...
    """
    clean_data = flatten(remove_nulls(data_blob), [])
    reviews = list(set([val for val in clean_data if is_clean_text(val, vocab)]))
    return reviews
...
def extract_place_data(html_content):
    ...
    voc_df = pd.read_csv("vocabs.csv")
    vocab = set(voc_df['word'].tolist())
    ...
    place_details = {
        ...
        "reviews": get_reviews(data_blob, vocab),
